...
test('clausula 5.8 ALTERAÇÃO DE PEDIDO presente na lista padrão', () => {
  const lista = getClausulasDefault();
  const c = lista.find(x => x.numero === '5.8');
  assert(c, 'faltou a cláusula 5.8');
  assert(c.titulo === 'ALTERAÇÃO DE PEDIDO', `titulo errado: ${c.titulo}`);
...
test('clausula 5.10 CANCELAMENTO DE PEDIDO presente na lista padrão', () => {
  const lista = getClausulasDefault();
  const c = lista.find(x => x.numero === '5.10');
  assert(c, 'faltou a cláusula 5.10');
  assert(c.titulo === 'CANCELAMENTO DE PEDIDO', `titulo errado: ${c.titulo}`);
  assert(c.conteudo.includes('produzidos sob encomenda'), 'conteudo da cláusula 5.10 incompleto');
});

test('clausula 5.11 NÃO ALICIAMENTO presente na lista padrão', () => {
  const lista = getClausulasDefault();
  const c = lista.find(x => x.numero === '5.11');
  assert(c, 'faltou a cláusula 5.11');
  assert(c.titulo === 'NÃO ALICIAMENTO E NÃO CONTRATAÇÃO DE PESSOAL', `titulo errado: ${c.titulo}`);
  assert(c.conteudo.includes('não aliciar'), 'conteudo da cláusula 5.11 incompleto');
});

for (const [rotulo, custom] of [['banco', comoBanco], ['default', comoDefault]]) {
  test(`[${rotulo}] o título da 5.24 é editável (data-clausula-campo="titulo")`, () => {
    const html = gerar(custom);
    assert(/<h3 data-clausula-campo="titulo">5\.24 PREÇO, CONDIÇÃO DE PAGAMENTO E IMPOSTOS<\/h3>/.test(html),
      'o título da 5.24 não está num elemento editável');
  });

  test(`[${rotulo}] o texto de introdução da 5.24 é editável (data-clausula-campo="conteudo")`, () => {
    const html = gerar(custom);
    const sec = html.match(/<section[^>]*data-clausula-slot="24"[\s\S]*?<\/section>/);
    assert(sec, 'não achei a seção de texto da 5.24 marcada com data-clausula-slot="24"');
    assert(/data-clausula-key="/.test(sec[0]), 'a seção da 5.24 não tem data-clausula-key');
  ...
  test(`[${rotulo}] a CONDIÇÃO DE PAGAMENTO é editável e tem key própria`, () => {
    const html = gerar(custom);
    const secs = html.match(/<section[^>]*data-clausula-slot="24"[^>]*>/g) || [];
    assert(secs.length >= 2, `esperava 2 blocos editáveis na 5.24 (intro e condição), veio ${secs.length}`);
    const keys = secs.map(s => (s.match(/data-clausula-key="([^"]+)"/) || [])[1]);
    assert(new Set(keys).size === keys.length, `keys repetidas na 5.24: ${keys.join(',')}`);
  ...
  test(`[${rotulo}] I4: a tabela de preços fica FORA dos containers editáveis e ANTES da condição`, () => {
    ...
    assert(iIntro > -1 && iTabela > -1 && iCondicao > -1, 'sumiu alguma parte da 5.24');
...
test('proposta LEGADA (cláusulas salvas antes, sem 5.24 no banco) ainda mostra os textos da 5.24', () => {
  const legado = comoBanco.filter(c => !/^5\.24/.test(c.titulo) && !/^5\.23 PREÇO/.test(c.titulo));
  const html = gerar(legado);
  assert(html.includes('5.24 PREÇO, CONDIÇÃO DE PAGAMENTO E IMPOSTOS'), 'o título da 5.24 sumiu na proposta legada');
  assert(html.includes('A CONTRATANTE pagará pelos equipamentos'), 'a introdução da 5.24 sumiu na proposta legada');
  assert(html.includes('Primeira Parcela/Entrada'), 'a condição de pagamento sumiu na proposta legada');
  assert(/data-clausula-slot="24"[^>]*data-clausula-key="temp-|data-clausula-key="temp-[^"]*"[^>]*data-clausula-slot="24"/.test(html),
    'na proposta legada os blocos da 5.24 precisam de key temp-* para que a 1ª edição vire cláusula nova no banco');
});
...
test('dado REAL da proposta 74: cláusula salva como "5.23 CONSIDERAÇÃO FINAL" vai para a página de assinatura', () => {
  ...
  const iGrupo524 = src.indexOf('five-24-preco-group');
  ...
  assert(iGrupo524 < iConsideracao, 'a CONSIDERAÇÃO FINAL ficou ANTES da seção 5.24 (I2)');
  assert(iAssinatura > -1 && iConsideracao > iAssinatura, 'a CONSIDERAÇÃO FINAL não foi para a página de assinatura');
  assert(html.includes('A CONTRATANTE pagará pelos equipamentos'), 'a introdução padrão da 5.24 sumiu');
});

test('a 5.24 aparece uma única vez (não duplica entre a lista de cláusulas e a seção de preço)', () => {
...
test('round-trip do título: o que o editor lê de volta é EXATAMENTE o título salvo', () => {
  ...
  const html = gerar(comoBanco);
  const secoes = html.match(/<section[^>]*data-clausula-slot="24"[\s\S]*?<\/section>/g) || [];
  assert(secoes.length >= 2, `esperava 2 seções da 5.24, veio ${secoes.length}`);
...
test('o prefixo numérico do 2º bloco NÃO é exibido (o documento mostra só "CONDIÇÃO DE PAGAMENTO:")', () => {
  ...
  assert(!texto.includes('5.24.1'), 'o prefixo interno 5.24.1 vazou para o texto do documento');
});
