...
function initializeDatabase(onReadyCallback) {
  ...
  let seedAdminPassword =
    process.env.SEED_ADMIN_PASSWORD || (resolvedSeed && resolvedSeed.password) || null;
  const seedFromEnv = Boolean(process.env.SEED_ADMIN_EMAIL && process.env.SEED_ADMIN_PASSWORD);

  db.get('SELECT * FROM usuarios WHERE email = ?', [seedAdminEmail], (err, row) => {
    if (row) {
      return;
    }
    if (!seedAdminPassword) {
      seedAdminPassword = generatePassword();
      persistSeedAdmin(PERSISTENT_DATA_DIR, seedAdminEmail, seedAdminPassword);
    }
    const hashedPassword = bcrypt.hashSync(seedAdminPassword, 10);
    const autoGenerated = !seedFromEnv && !resolvedSeed;
    db.run(
      'INSERT INTO usuarios (nome, email, senha, cargo, role, is_superadmin) VALUES (?, ?, ?, ?, ?, ?)',
      ['Administrador', seedAdminEmail, hashedPassword, 'Administrador', 'admin', 1],
      (insertErr) => {
        if (insertErr) {
          console.error('Erro ao criar usuário admin:', insertErr);
        } else if (autoGenerated) {
          console.log('Admin inicial criado - credenciais em server/data/.runtime-secrets.json');
        } else {
  ...
  db.all("PRAGMA table_info(usuarios)", (err, rows) => {
    ...
    if (rows && rows.length > 0) {
      ...
      db.run(
        ...
        (superErr) => {
          if (!superErr) console.log('✅ Super administrador padrão verificado');
        }
      );
    ...
    ${forPdfServer
      ...
      <div class="section">
        ...
        <div class="texto-corpo" style="line-height: 2;">
          ...
          <p>5.9 DEVOLUÇÃO OU TROCA DE MERCADORIA ................................................................. 13</p>
          <p>5.10 CANCELAMENTO DE PEDIDO .................................................................................... 14</p>
          <p>5.11 NÃO ALICIAMENTO E NÃO CONTRATAÇÃO DE PESSOAL ........................................ 14</p>
...
function initChatModule() {
  if (!db || chatModuleRegistered) return;
  ...
  initChatSchema(db)
    ...
    .catch((err) => console.error('❌ Erro ao inicializar módulo Chat:', err));
}
