...
| `POST /api/almoxarifado/ferramentas/:id/ocorrencias` | gerenciar_ferramentas | multipart: `tipo!` (`AVARIA\|PERDA`), `descricao!`, `responsavel_nome?`, `responsavel_colaborador_id?`, campo de arquivo `foto?` | 201 `{id}` | 400 `"Tipo de ocorrência inválido"` · 400 `"Ferramenta não pode registrar ocorrência (status atual: <STATUS>)"` (origens válidas: só `DISPONIVEL`\|`EMPRESTADA`) · sobre EMPRESTADA, RN-05 aplica sem recusa extra. *(Correção declarada: esta linha dizia só "RN-05 aplica sem recusa extra" e omitia a recusa por status — a revisão da Task 5 flagrou a lacuna; a recusa sempre foi mandada pelo brief da task.)* |
