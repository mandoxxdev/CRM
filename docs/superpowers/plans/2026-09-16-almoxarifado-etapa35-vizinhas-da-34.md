...
| RN-03 | `integração: clique, foco da janela e troca de filtro — uma carga por gesto` (novo) | `RequisicoesList.test.js` | `toBe(1)` → `toBe(2)` → `toBe(3)` nos três passos | `window.dispatchEvent(new Event('focus'))` e o 1º `input[type="checkbox"]` (molde `:583-614`) |
| RN-04 | `(h) a lista que NAO carregou mostra erro, nunca "Nenhum recebimento registrado"` (novo) | `RecebimentosAlmoxarifado.test.js` | `expect(container.textContent).not.toContain('Nenhum recebimento registrado')` | **sobrescrever** `api.get.mockImplementation` rejeitando `/almoxarifado/recebimentos` com `{ response: { data: { error: 'Sem acesso ao módulo' } } }` |
...
E no cenário de deep-link (`:503-518`), a régua da RN-02 — que já é verde hoje e **tem de continuar**
...
```js
  useEffect(() => {
    ...
    // já foi carregado pelo clique e um segundo GET é desperdício puro. Desarma sempre aqui — este
    // é o ÚNICO ponto de desarme, e por isso a flag nunca sobrevive a um ciclo do efeito.
...
| 3 | fazer `syncSearchParams` devolver `next` **sempre** (mesmo quando não escreve), mantendo a comparação por identidade | **previsão: NADA cai** — e isso é um achado a reportar, não um detalhe. Com a comparação por identidade, armar sem ter escrito só pode ser consumido por uma query **idêntica**, isto é, pelo mesmo id e mesmos filtros, estado em que o detalhe já está carregado: o defeito ficou **inalcançável** (caso (2) da regra da `fechar-etapa`). **Mantenha a guarda** — ela é barata e é o que torna o comportamento legível — e **declare que a suíte não a protege**. Se, ao contrário da previsão, algum cenário cair, registre qual: a previsão é que está errada. |
...
|---|---|---|
| 1 | apagar `setErro(msg)` do `catch` (deixando o `toast.error`) | **(h)** por `toContain('Não foi possível carregar os recebimentos.')` **e** por `not.toContain('Nenhum recebimento registrado')` — as duas, e é isso que prova que o conserto é o DOM e não o toast. **(i)** também cai. |
| 2 | **sabotagem de POSIÇÃO:** mover o ramo `erro ?` para **depois** de `recebimentos.length === 0 ?` | **(h)** cai pela frase de erro ausente; **(i)** cai por `linhas()` — com a lista velha em memória, a tabela volta a renderizar. É o controle da ordem, que nenhuma asserção de presença sozinha pega. |
...
| — | apagar `setRecebimentos([])` do `catch` | **previsão: nada cai**, porque o ramo `erro ?` precede a tabela e a lista velha nunca chega a renderizar. **Mantenha** (é o molde de `HistoricoInspecoes.js:72` e a defesa em profundidade de RN-05) e **declare que a suíte não o protege** — caso (2) da regra da `fechar-etapa`. |
...
**Files:**
- Modify: `client/src/components/almoxarifado/RecebimentosAlmoxarifado.js:53-56, :111-143, :443, :469, :485, :489-494, :588-604`
- Modify: `client/src/components/almoxarifado/RecebimentosAlmoxarifado.test.js`

**Interfaces:** nenhuma para fora; o bloco de anexos continua
`<AnexosDocumento entidade="recebimento" entidadeId={detalhe.id} titulo="Anexos" />` dentro de
`{detalhe && …}` — e é **por isso** que anular `detalhe` conserta o defeito: o bloco sai de cena com
o id antigo em vez de ficar oferecendo upload para o registro errado.

⚠️ **Leia a regra (iv) antes de escrever código.** O cabeçalho do painel desreferencia
`detalhe.numero` (`:489`) e `detalhe.status` (`:490-491`) **sem guarda**. Anular `detalhe` mantendo
`{detalhe && …}` faz o painel inteiro piscar para fora; anular sem trocar o gate por `selectedId`
**e** sem o `?.` no cabeçalho lança `TypeError` no primeiro render e derruba **todos** os cenários
juntos — sabotagem acidental que não prova nada. O molde completo é `RequisicoesList.js`:
`selectedId` (`:100-103`) setado sincronamente em `abrirDetalhe` (`:232`), painel por `{selectedId &&`
(`:852`), corpo por `loadingDetalhe || !detalhe` (`:876`) e cabeçalho `detalhe?.numero || '...'`
(`:856`).
...
Os cinco pontos de render (todos mecânicos; **nenhum** pode ficar de fora, e o terceiro sem o
quarto é `TypeError`):

| Linha de hoje | Passa a ser |
|---|---|
| `:443` `gridTemplateColumns: detalhe ? '1fr 420px' : '1fr'` | `selectedId ? '1fr 420px' : '1fr'` |
| `:469` `background: detalhe?.id === r.id ? …` | `background: selectedId === r.id ? …` |
| `:485` `{detalhe && (` | `{selectedId && (` |
| `:489-491` `{detalhe.numero}` e a badge | `{detalhe?.numero \|\| '...'}` e `{detalhe && <span className={…}>…</span>}` |
| `:494` `onClick={() => setDetalhe(null)}` | `onClick={fecharDetalhe}` |

- [ ] **Step 4: reescrever o comentário de `:588-604`.** Ele diz hoje: *"o `id` não muda durante o
...
**Interfaces:** `.almox-actions` é **global do módulo** — 11 telas, 18 ocorrências. Nenhuma tela
ganha classe nova.
...
`769 − 48 = 721px` (`.almox-page` padding, `:7-9`) e a célula de ações **sozinha** pede ~685px
(textos a 6,5px/char + 5 × [padding 32 + borda 2 + ícone 13 + gap interno 7] + 4 × gap 6 + `td`
padding 32). Sobram **36px** para as seis outras colunas de
...
perl -e 'my @t=(9,8,18,10,10); my $c=6.5; my $fix=32+2+13+7;
  ...
  printf("celula de acoes (Ferramentas, pior caso): %.0fpx\n", $cel);
...
.almox-actions {
  ...
  /* Etapa 35: sem `flex-wrap`, a celula de acoes estourava a largura da tabela e o
     ...
     `white-space: nowrap` (:368) tem piso duro de ~685px numa largura util de 721px em 769px —
...
| # | Sabotagem | O que TEM de cair |
|---|---|---|
| 1 | remover `flex-wrap: wrap` (voltar ao estado anterior) | asserção 1 — é o controle de que o teste guarda o conserto, e não uma frase sobre ele |
| 2 | `gap: 6px` → `gap: 8px` em `.almox-actions` | asserção 2. Sem ela, a conta documentada (685px) envelheceria em silêncio |
| 3 | remover `white-space: nowrap` de `.btn-almox-secondary` | asserção 4 — a premissa que **decidiu** a etapa. Se ela cair e ninguém notar, a próxima sessão lê a medição como se ainda fosse piso duro |

`grep -cF` da âncora = 1 antes de cada `perl -0pi -e`; `md5sum` nos três momentos;
 ...
 */
test('integração: clique, foco da janela e troca de filtro — uma carga por gesto', async () => {
  ...
  // e o detalhe É recarregado. É aqui que uma flag armada indevidamente no passo 2 apareceria.
  ...
  // E o bloco de anexos atravessou os três gestos sem remontar nem reconsultar (F2 da Etapa 34).
  expect(blocoDeAnexos()).not.toBeNull();
...
- [ ] **Step 2: rodar e ver passar.** Se o passo 3 devolver 2, a flag sobreviveu ao passo 2 — é o
      ...
      `null` quando não escreve **e** que a comparação do efeito é por igualdade de string).
- [ ] **Step 3: reexecutar a sabotagem 2 da T2 contra ESTE cenário** e confirmar que ela derruba o
      passo 3 (`Expected length: 3 / Received length: 2`). É o controle positivo que a T2 não podia
      ter sozinha, porque o cenário não existia ainda. Se ela **não** derrubar, o cenário está
      medindo outra coisa — conserte o cenário.
...
- Modify: `docs/almoxarifado-novidades-por-etapa.md` (seção da Etapa 35, letra B, e a seção final
...
- [ ] **Step 5: letra B** do `almoxarifado-novidades-por-etapa.md` com as **sete** decisões do
      ...
      `grep -o "C4[0-9]" docs/almoxarifado-novidades-por-etapa.md | sort -u`).
...
- **Cobertura RN ↔ task ↔ sabotagem.** RN-01 → T1 (vermelho medido) + T2 (verde) + sabotagem 1 da
  ...
  deep-link"; RN-03 → T6 passos 2 e 3 + sabotagem 2 da T2 (que só pode ser exercida contra o
  cenário da T6 — está dito na T6, Step 3); RN-04 → T3 cenário (h) + sabotagens 1 e 2; RN-05 → T3
  cenário (i) + sabotagem 2 (posição); RN-06 → T3 cenário (j) + sabotagem 3; RN-07 → T4 cenário (k)
...
- **As duas sabotagens previstas como "nada cai" estão declaradas como tal** (T2 nº 3 e a quarta
  linha da tabela da T3), com o motivo (defeito inalcançável) e a instrução de **manter** a guarda e
  **declarar** que a suíte não a protege. Isso é o caso (2) da regra da `fechar-etapa`, não uma
  lacuna escondida.
- **Nenhum cenário negativo sem metade positiva.** (h) tem `Recebimentos NF`; (i) tem a contagem de
  ...
  `REQ-055` e `toHaveLength(0)`; o de CSS tem a guarda-da-guarda (`css.length > 20000`).
