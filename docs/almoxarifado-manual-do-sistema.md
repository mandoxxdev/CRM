...
- **Configuração registra só o que mudou, e o salvamento é tudo ou nada.** A tela de configurações envia todos os campos a cada salvamento; o histórico guarda **uma** linha por salvamento, contendo apenas os campos que de fato mudaram de valor. Salvar sem alterar nada não gera registro. E o salvamento é **indivisível**: os campos são gravados num único comando, então **ou todos são aplicados ou nenhum é**. Se o salvamento falhar, o aviso de erro descreve o estado real — nada foi alterado, e por isso não há registro a fazer.
- **Ato sem efeito não vira registro.** Excluir um cadastro do módulo (tipo de material, localização, setor, família) e desativar um material são **inativações**: a linha sai das listas e continua no banco. Clicar em Excluir de novo sobre algo que **já estava inativo** é aceito e responde normalmente — a tela permite isso de propósito, porque duas abas abertas ou um F5 são situações comuns —, mas **não gera nenhuma linha nova no histórico**, porque nada mudou. Consequência para quem audita: **uma linha de "Exclusão" na trilha significa sempre uma exclusão que teve efeito.** Repetir o clique não infla o histórico, e a diferença entre um item excluído uma vez e um item excluído três vezes não existe na trilha, porque também não existe no cadastro. Um item que **nunca existiu** é caso diferente e continua sendo recusado com aviso próprio.
