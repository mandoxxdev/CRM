...
- [ ] Dupla conferência em materiais críticos (feature 05) — **o único item da perna *Segurança*
...
| **Segurança** | ~~**Registrar dispositivo (user-agent/IP) na movimentação**~~ **PAGO na Etapa 25 (`9027c36`)** · ~~**bloquear lançamento retroativo**~~ **NUNCA FOI TAREFA — esta linha estava ERRADA**, ver o item `[~]` no checklist · ~~justificativa obrigatória em operações excepcionais~~ **JÁ ESTAVA CONSTRUÍDA em 5 camadas — esta linha estava ERRADA**; o que falta virou lista nomeada · **dupla conferência em materiais críticos** (decisão de negócio, letra **B**) · ~~política de retenção de backup configurável~~ **PAGA na Etapa 25 (`6209037`+`d81191e`)** | 5 itens → **2 pagos, 2 eram classificação errada, 1 é decisão de negócio** | **Não** (nenhum item restante é funcionalidade não construída) |
