...
| 07 | [Reservas de estoque](07-reservas/README.md) | 🟢 | ❌ | 🟢 | 🟡 **Etapa 4: backend entregue (2026-08-05)** — consumo contra reserva (o buraco central: antes reservar tornava o saldo inutilizável até para quem reservou), reserva automática na aprovação com os status PARCIALMENTE/TOTALMENTE_RESERVADA, transferência entre projetos, expiração por endpoint (opt-in) e liberação no cancelamento da requisição. **Falta a tela de reservas** — é a próxima tarefa. Falta também a lane `/aprovar-valor` reservar |
