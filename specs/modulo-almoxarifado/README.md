...
> **Última atualização:** 2026-09-22 (**Etapa 41 fechada — a cotação ganhou ITENS e vira PEDIDO de
> compra, `7d9e7d7..ffba9b9`** — T1–T5 mais a onda de correção F1–F6. Quarta etapa seguida no módulo
> **core Compras**, e a que fecha o "próximo alcançável" nomeado no fechamento da 40: a cotação da 40
> era cabeçalho chapado com `valor_total` digitado, e aprovada ela obrigava o comprador a **redigitar**
> o pedido inteiro. É a **fatia Compras da feature 22**; a spec continua lá.
> **Entregue:** (1) **o tronco** (`8d81cc5`): `itens_cotacao` em `schema.js` (não em `index.js` —
> `initSchema` roda no harness e a tabela chega a toda suíte com a DDL de produção, sem stub; a
> classe de divergência da F1 da 40), espelho de `itens_pedido_compra` **sem** `quantidade_recebida`
> e com `valor_unitario` de mesmo nome para o custo médio do recebimento herdar; `CotacaoItemSchema`
> com `itens` **OPCIONAL** (a cotação de cabeçalho continua válida; quem exige itens é a conversão);
> `resolverItens` exportada do serviço de pedido; `cotacoes.pedido_id` por `ALTER` em três lugares.
> (2) **O servidor** (`11591ca`): `valor_total` com **duas regras** (derivado com itens, digitado sem),
> `DELETE /api/compras/cotacoes/:id` **próprio** acima do genérico (com item, o genérico daria 500 em
> produção e passava no harness deixando órfãos — a sabotagem obrigatória conta órfãos), `POST
> /api/compras/cotacoes/:id/gerar-pedido` chamando o `criarPedido` da 38 (`PC-…`, total derivado,
> `data_pedido` hoje local — a Fase 2 achou que nascia NULL), `status = 'aprovado'` ao converter,
> 409 na segunda, 400 para rejeitada/cancelada/sem itens/**fornecedor inativo** (a primeira porta do
> Compras a olhar `status`), convertida não se edita (409, RN-F15) e **excluir o pedido libera a
> cotação** (a Fase 2 inverteu a RN-F12 original, que deixava a cotação num beco). (3) **A tela da
> cotação** (`7ecf91f`): bloco de itens copiado do pedido, *Valor total* **travado** com a soma
> enquanto há linha e digitável sem — caminho que preserva os 8 cenários da 40 sem tocar fixture.
> (4) **A aba** (`bd224d2`): coluna **Pedido** com link, botão **"Gerar pedido"** condicional, toast
> e `navigate` para a edição do pedido, `Pedido` no fim do Excel. (5) **A integração** (`dae1cee`)
> pela rota e pelo serviço até o aux do recebimento da 37 com `saldo_pendente` cheio, segunda
> conversão 409, exclusão do pedido liberando, regeneração com `PC-` novo e exclusão final com 0
> órfãos.
> **A REVISÃO da Fase 2 (plano, antes de codar) achou 1 Critical, 4 Important e 8 Minor, todos
> aplicados em `183ac38`; a REVISÃO FINAL (2 lentes independentes) achou 1 Critical, 4 Important e
> 7 Minor, zero ruído** — o Critical da lente UX e o I1 da lente RN são o **mesmo achado**: a
> conversão **não era atômica**. A onda F1–F6 (6 commits): **F1** (`d6a1beb`) — 6 `POST …/gerar-pedido`
> em paralelo criavam **6 pedidos**, 5 sem cotação apontando e todos visíveis no aux do recebimento
> (duplo clique reproduzia com 2); o `UPDATE` do vínculo virou compare-and-set (`AND pedido_id IS
> NULL`) e o perdedor é compensado com `excluirPedido` + 409 com o vencedor; descartada a fila em
> memória. **F2** (`83a5d71`) — a suíte era **cega** para a ordem UPDATE → DELETE: mover o `UPDATE
> cotacoes SET pedido_id = NULL` para depois do `DELETE` do pedido ficava verde nas 20 e dava 500 em
> produção; `comprasCotacaoFkProducao.api.test.js` abre um segundo banco com a DDL de produção
> **lida** de `index.js`/`schema.js`, `foreign_keys = ON` com controle positivo dentro do cenário, e
> prova as duas lixeiras e a compensação do F1. **F3+F3b** (`ffba9b9`) — o cabeçalho do pedido gerado
> não era afirmado; `somaItens` arredondada a 2 casas. **F4** (`a09dfe8`) — o botão aceitava clique
> repetido em voo; `useRef`, porque a guarda por estado não protegia (sabotagem verde). **F5**
> (`abb46f4`) — a convertida abria em edição livre e perdia o trabalho no Salvar; agora abre travada
> com faixa e link. **F6** (`327d33f`) — 3 × 0,1 mostrava `0.30000000000000004` no campo travado.
> **F7** (`68d4173`, da re-revisão da onda — 0C/1I/4M): apagar a cotação com o `gerar` em vôo
> deixava pedido órfão em 28 de 301 janelas; a exclusão passa a reivindicar por CAS antes de apagar e
> o CAS do `gerar` recusa cancelada.
> **Números medidos no fechamento** (tronco `68d4173`): `test:api` **195/195 arquivos** (eram 191);
> almoxarifado **42/0**; validation **4/0**; safealter **3/0**; sqlite **5/0**; client **51 suítes /
> 781 testes** (eram 769); build **Compiled successfully**.
> **O que é do usuário:** as consultas **A18/A19** (`PRAGMA table_info` de `itens_pedido_compra`
> **9**, `itens_cotacao` **8** e `cotacoes` **11** depois do primeiro boot — o dump de 03/09 tem
> 8/–/10; e órfãos em `itens_cotacao` = 0) e as decisões **B141 em diante** (D1–D12 do design mais
> as da Fase 2 e da onda: liberar a cotação ao excluir o pedido; convertida não se edita; CAS +
> compensação em vez de fila; o pedido regenerado nasce da cotação, não do pedido editado; sem
> `window.confirm` no "Gerar pedido"; `criarPedido` segue somando cru).
> **O que fica declarado (letra G):** comparar cotações **não existe** (é o que sobra alcançável na
> fatia); o `ALTER … pedido_id` falha no **primeiro** boot de banco **novo** (fila do sqlite3 fora do
> `serialize`, padrão herdado das cinco colunas de `fornecedores`); um entrelaçamento teórico
> `excluirCotacao` × `gerar` fica fora do CAS (o (12) acusa se aparecer); `criarCotacao` pelo serviço
> não passa pelo schema; a lixeira da aba segue visível para a convertida; `assertFornecedor` não
> checa status nas portas de pedido; a frase morta de `comprasPedidoEditarExcluir:93` continua.
> Antes: 2026-09-22 (**Etapa 40 fechada — Fornecedores e Cotações ganharam TELA:
> os quatro caminhos mortos do Compras, `7ccfc85..03cd048`** — T1–T6 mais a onda de correção F1–F5.
> Terceira etapa seguida no módulo **core Compras**, e a que fecha as duas abas que a 38 e a 39
> deixaram: o botão "Novo" e o lápis das abas Fornecedores e Cotações voltavam para a lista desde
> sempre. É a **fatia Compras da feature 22**; a spec continua lá.
> **Entregue:** (1) **o tronco** (`008a041`): `FornecedorSchema` — um **retrofit** sobre duas portas
> que existiam sem Zod, cujo contrato é **não recusar** os quatro payloads reais do modal de
> `FornecedoresDoGrupo` (`''` nos textos, `grupo_id` string de `useParams`, `null` no "Remover") —,
> `CotacaoSchema` com `numero` **digitado** (é o número do documento do fornecedor), `erro`/
> `assertFornecedor` exportados do serviço de pedido e `cotacoes` no harness. (2) **O servidor de
> fornecedor** (`6795b39`): `GET /api/compras/fornecedores/:id` **não existia** — criado com projeção
> nomeada, sem `planilha_*`; **nenhuma porta escrevia `status`** (o filtro *Inativo* da aba e três
> `WHERE status = 'ativo'` eram **inertes**) — o `PUT` passou a escrevê-lo; e **"Remover do grupo" era
> no-op** (`grupo_id: null` virava "ausente" na rota) — passou a limpar. (3) **O servidor de cotação**
> (`29dd6a8`): `cotacaoService.js` e `POST`/`GET /:id`/`PUT`, com o 409 do número em **duas guardas**
> (antes do `INSERT` e no `catch` do `UNIQUE`). (4) **As duas telas** (`23b86f3`, `b692413`):
> `FornecedorForm` (7 textos, grupo, **status só na edição**) e `CotacaoForm` (cabeçalho: número,
> fornecedor, datas, valor digitado, status, observações), erro do servidor em `role="alert"`, toast
> só no sucesso, rótulo *"Nova Cotação"* (era *"Novo Cotação"*) e `<select>` de status por aba — com
> `statusValido` derivado, porque `<Compras/>` **não remonta** entre abas e o filtro *Inativo* viajaria
> para Cotações mostrando "Todos". (5) **A integração pela rota e pelo serviço** (`d64ede0`), com o
> controle positivo do seletor do recebimento **antes** da negativa (I2 da Fase 2: sem ele, um aux
> vazio provaria RN-E05 sem provar nada).
> **A REVISÃO da Fase 2 (plano, antes de codar) achou 0 Critical, 4 Important e 9 Minor, todos
> aplicados em `84ace0b`; a REVISÃO FINAL (2 lentes independentes) achou 0 Critical, 3 Important e
> 10 Minor, zero ruído.** A onda F1–F5 (6 commits): **F1** (`bdaadd8`) — `fornecedores` tem **três**
> FKs e o 409 da lixeira contava duas: fornecedor com lista de preços (`itens_fornecedor`) e sem
> pedido dava **500** em produção, o defeito que a F5 da 38 dizia ter fechado; terceira contagem +
> stub no harness. **F2** (`55a3214`) — o cenário (a) de `CotacaoForm.test.js` calculava o esperado
> com o mesmo `new Date()` do componente e passava com a implementação UTC das 00:00 às 20:59;
> reescrito com relógio fixo às 23:30. **F3** (`8cde2ee` + `795e47d`) — fornecedor inativado dentro
> de um grupo **sumia** do grupo (a rota filtrava `ativo`) sem selo e sem "Remover"; a rota devolve
> todos os status, ativos primeiro, e a tela pinta o selo e tira inativos do "Vincular". **F4**
> (`01732dd`) — `min="0"` no valor barrava o submit com tooltip nativa e a literal do servidor nunca
> chegava à faixa. **F5** (`03cd048`) — `grupo_id` acima de 2^53 saía em inglês.
> **Números medidos no fechamento** (tronco `03cd048`): `test:api` **191/191 arquivos**; almoxarifado
> **42/0**; validation **4/0**; safealter **3/0**; sqlite **5/0**; client **51 suítes / 769 testes**;
> build **Compiled successfully**.
> **O que é do usuário:** as consultas **A16/A17** (nenhum `status` fora de `ativo`/`inativo` ou
> `NULL`; nenhum `numero` de cotação repetido) e as decisões **B123–B140** (D1–D14 do design mais as
> da onda: 409 de itens em vez de cascata, `NULL` ordena com os ativos, inativo fora do "Vincular").
> **O que fica declarado (letra G):** itens de cotação e converter cotação em pedido **não existem**
> (não há `cotacao_itens`); `assertFornecedor` não checa status (pedido e cotação aceitam fornecedor
> inativo); a lista de fornecedores segue `SELECT *` com `planilha_dados`; a tradução do `UNIQUE` do
> número só é exercida por corrida; `dataIsoOpcional` aceita `2026-13-45` (regra de forma, herdada do
> pedido); o filtro de status **válido** nas duas abas viaja entre elas; `FornecedoresDoGrupo.js`
> continua **sem suíte**; `PUT /cotacoes/:id` valida o corpo antes do 404; `observacoes: ''` grava `''`.
> Antes: 2026-09-17 (**Etapa 39 fechada — o pedido de compra passou a ser
> ACOMPANHADO: prazo, atraso e os resíduos da 38, `39ea9d2..19ebf7d`** — T1–T5 mais a onda de
> correção F1–F4. Segunda etapa seguida no módulo **core Compras**, com **uma** entrada nova no
> registro de alertas do almoxarifado; é a **fatia Compras da feature 22** mais a feature **20**.
> **Entregue:** (1) **o atraso DERIVADO na leitura** (`437aed2`): `GET /api/compras/pedidos` devolve
> `atrasado: 0|1` e `dias_atraso: number|null` por linha, mais o filtro `?atrasados=1` — **nenhuma
> coluna, nenhuma migration, nenhum `UPDATE` de varredura**, porque coluna gravada fica errada à
> meia-noite todo dia. A régua é **uma função em um lugar** (`derivarAtraso` +
> `STATUS_PEDIDO_FORA_DO_ATRASO` + `hojeLocalISO`, exportadas de `pedidoCompraService.js`), e a
> fronteira é `<`: **vence hoje NÃO está atrasado**. (2) **O defeito escapado da Etapa 38**
> (`235c067`): `formatDate` da aba Compras era `new Date(str).toLocaleDateString('pt-BR')`, e
> `new Date('2026-09-16')` é **meia-noite UTC** — a tela mostrava **`15/09/2026`** em quatro campos
> **e no Excel**, e como a importação lê `DD/MM/AAAA`, **exportar e reimportar o próprio Excel do CRM
> movia as datas um dia para trás a cada volta**. Agora formata a string por `split('-')`; o
> `hojeISO` do formulário virou local (era UTC: entre 21h e 0h o pedido nascia com a data de amanhã).
> (3) **A tela** (`833850e`): badge vermelho `Atrasado há 1 dia`/`Atrasado há N dias`, checkbox
> `Só atrasados` e as colunas `Atrasado` (`Sim`/`Não`) e `Dias de atraso` **no fim** do Excel — a
> posição é contrato, não estética (há um leitor guloso de planilha que pega o primeiro número da
> linha). (4) **O alerta `PEDIDO_COMPRA_ATRASADO`** (`ddbc18f`): **uma** entrada no `alertRegistry`
> liga varredura diária, dedupe, e-mail e central de uma vez — e é a **primeira entrada do registro
> que lê tabelas CORE**, consumindo a **mesma** `derivarAtraso` por require **lazy** (o ciclo de
> módulos está a um require de distância de fechar, e quando fechar um lado captura `{}` em silêncio).
> (5) **A integração pela ROTA e pelo JOB** (`fc84e09`), incluindo o recebimento real da Etapa 37
> pelas seis portas. **(6) `PATCH /api/compras/pedidos/:id/status`** (`19ebf7d`).
> **A REVISÃO FINAL (2 lentes independentes) achou 1 Critical + 4 Important, todos reproduzidos por
> sonda, e 13 Minor; nenhum ruído.** **F1** (`45bda69`): em produção o contêiner é `node:20-alpine`
> **sem `TZ` e sem `tzdata`**, então `hojeLocalISO()` era **UTC** e às 21:30 BRT o pedido que vence
> **hoje** já acusava atraso no badge, no filtro, no Excel e no e-mail — as suítes passavam iguais
> porque rodam em `TZ` fixo. Agora o dia é recortado por `Intl` em `America/Sao_Paulo`, mais
> `tzdata` + `ENV TZ` no `Dockerfile` (as duas, nunca uma). **F2** (`33031ac`): o dedupe só pelo id
> calava o pedido **para sempre** — prazo renegociado e furado de novo nunca gerava segundo aviso;
> a chave passou a incluir a previsão. **F3** (`8f3db94`): `SELECT p.*` punha `valor_total` e
> `observacoes` de pedidos CORE na central de alertas, cujo gate **não** inclui o módulo Compras —
> colunas nomeadas. **F4** (`19ebf7d`): **todo pedido recebido pelo almoxarifado ficava "Atrasado"
> para sempre**, porque a guarda da 38 recusa o `PUT` e nada mais muda o status; a saída foi uma
> **porta que só escreve status**, com `teve_recebimento` no `GET /:id` e a faixa *"Este pedido já
> teve recebimento — só o status pode ser alterado"* na tela. **Descartado: afrouxar a guarda** — o
> `PUT` faz `DELETE`+`INSERT` das linhas e zeraria `quantidade_recebida`, fazendo o operador receber
> o mesmo material duas vezes.
> **Números medidos no fechamento:** `test:api` **187/187 arquivos**; almoxarifado **42/0**;
> validation **4/0**; safealter **3/0**; sqlite **5/0**; client **49 suítes / 753 testes**; build
> **Compiled successfully**.
> **O que é do usuário:** a consulta que diz se o alerta deixou de ser inerte (`COUNT(*)` de
> `pedidos_compra` com previsão preenchida) e a confirmação do `TZ` do processo em produção.
> **O que fica declarado (letra G):** o `PATCH` aceita **qualquer** status, inclusive em pedido
> recebido (não há máquina de estados em Compras); a hora da varredura diária é a hora em que o
> contêiner subiu e **anda a cada deploy**; a exportação da aba Pedidos segue em **N+1**; a central
> mostra a previsão como `25/09/26` e a aba como `25/09/2026`; `GET /api/compras/pedidos/:id` **não**
> deriva atraso; e a tela de recebimento da Etapa 37 **não** mostra o atraso.
> Antes: 2026-09-16 (**Etapa 38 fechada — o pedido de compra GANHOU CRIAÇÃO, e a
> Etapa 37 deixou de ser inerte, `be71754..0a7e5c6`** — T1–T7 mais a onda de correção F1–F8. É a
> **fatia Compras da feature 22** e é a primeira etapa deste projeto num módulo **core**, não no
> almoxarifado: a tela mora em Compras → Pedidos, e a spec dela vive na 22 como "fatia Compras"
> (B105, reversível).
> **Entregue:** (1) as **23 rotas `/api/compras/*` extraídas** de `server/index.js` para
> `server/routes/compras.js` e montadas no harness (`be71754`) — bloco de 498 linhas com **md5
> idêntico**, `grep -c` caindo de 26 para 3, e a **primeira suíte que bate em `/api/compras`**; com
> ela veio a **caracterização** de que o `DELETE /api/compras/:tipo/:id` genérico **sombreia**
> `grupos/:id` (400 *"Tipo inválido"*), congelada para detectar reordenação, **não consertada**.
> (2) **`POST /api/compras/pedidos`** (`fa410ce` + fix `3e43069`): os **primeiros schemas Zod do
> módulo core**, `numero` **gerado** (`PC-…`, gerador da 31 — nunca o do payload), `valor_total`
> **derivado**, materiais resolvidos **antes** de qualquer escrita. O **fix 1 reverteu a decisão 10
> do design**: o vínculo da solicitação escrevia em `solicitacoes_compra_almoxarifado` **sem**
> `gerenciar_reposicao`, e o furo era **alcançável por clique** — qualquer usuário do módulo
> Compras (o fallback `PRODUCAO` inclusive) flipava a solicitação para `VINCULADO`. Agora é
> **403 antes de escrever**, com o corpo idêntico ao de `requirePermission`.
> (3) **`GET /:id`, `PUT` e `DELETE`** (`6c21e89`) com a guarda de **DUAS pernas** — recebida > 0
> **OU** recebimento referenciando a linha por `pedido_item_id`, porque um recebimento **ABERTO**
> tem 0 recebido e **já carrega o elo**, e o elo **não tem FK**. Sonda executada com a perna 2
> removida: `PUT` 200, ids novos, material entrando no estoque e o pedido `ABERTO` com saldo cheio
> **para sempre**. (4) **`GET /api/compras/materiais`** e **`POST /api/compras/pedidos/importar`**
> (`877ef23`) — a busca própria existe porque `checkModulePermission('almoxarifado')` barra o
> **prefixo inteiro**, e a importação passa **cada grupo pelo mesmo `criarPedido`**, nunca por
> `INSERT` direto. (5) **O formulário e as duas rotas que faltavam** (`9f1a3da`): os dois `<Link>`
> mortos da aba Pedidos passaram a navegar e o 409 da exclusão deixou de morrer no
> `toast.error('Erro ao excluir item')` genérico — e *"a rota tem de vir antes do `path="*"`"*
> **era falso** (react-router 6 casa por **ranking**). (6) **"Gerar pedido" na Reposição**
> (`727ee29`), o **primeiro consumidor** que `vincularPedidoCompra` já teve, navegando (não
> postando) com os dados da linha. (7) **A integração pela rota E pelo serviço** (`dc60507`).
> **A REVISÃO FINAL (2 lentes independentes) achou 1 Critical + 7 Important, todos reproduzidos por
> sonda, e 11 Minor não acionados.** **F1** (`4a41119`): a importação **fundia fornecedores
> diferentes** num pedido só — o item da BETA gravado no pedido da ACME, e a conta a pagar da 37
> nasceria para o fornecedor **errado**. **F2** (`ca7956c`): apagar o pedido deixava a solicitação
> `VINCULADO` apontando para id apagado, e ela **nunca mais virava pedido** — agora volta a
> `PENDENTE` e a resposta traz `solicitacoes_liberadas: N`. **F3** (`2fb9f68`): a coluna `DATE`
> aceitava `''` e serial do Excel, então o alerta de atraso que é etapa própria **nasceria acusando
> os pedidos SEM previsão**. **F4** (`9d07dd1`), **F5** (`59abaea`: apagar fornecedor com pedido
> virou **409** em vez de 500 por FK — alcançável só desde esta etapa), **F6** (`ba6278e`:
> importação 100% recusada era anunciada **em verde**, e o Excel que o próprio módulo exporta **não
> se reimportava**), **F7** (`d9181e3`: pedido importado nascia **sem data**), **F8** (`0a7e5c6`).
> **Números medidos no fechamento:** `test:api` **183/183 arquivos**; almoxarifado **42/0**;
> validation **4/0**; safealter **3/0**; sqlite **5/0**; client **48 suítes / 739 testes**; build
> **Compiled successfully**.
> **O que é do usuário:** a consulta **A14** (acervo `COUNT(*)` de `pedidos_compra`/
> `itens_pedido_compra` antes e depois do deploy — era **0/0**) e as decisões **B100–B113**.
> **O que fica declarado (letra G):** o core Compras continua com **UMA** camada de autorização —
> quem tem o módulo `compras` cria, edita, importa e apaga pedido, e o **único** gate de perfil é o
> condicional do vínculo; planilha com **CNPJ só na 1ª linha** da OC perde as demais; **reimportar
> duplica** (sem idempotência); preço **negativo vira 0**; o `hojeISO` do formulário ainda é **UTC**;
> a exportação da aba Pedidos faz **1 GET por pedido**; e `?pendentes=1` **não filtra status**, então
> pedido `cancelado` continua no `<select>` do recebimento (porta da 37, proibida por contrato nesta
> etapa).
> Antes: 2026-09-16 (**Etapa 37 fechada — o pedido de compra passou a SABER
> quanto dele já chegou, e a tela deixou de impedir o recebimento parcial, `ea0aa4f..13ad237`. É
> FEATURE da 08** (servidor + client), a **terceira porta** que a Etapa 36 anunciou — e **não** é a
> mesma comparação: a 36 mede contra a `quantidade_esperada` do próprio documento, esta mede contra
> o **saldo do pedido de compra**.
> **Entregue:** (1) **onde o saldo mora** — `itens_pedido_compra.quantidade_recebida` e
> `recebimentos_material_itens_almoxarifado.pedido_item_id` por `safeAlter`, mais o índice por
> `pedido_id` e o stub de `pedidos_compra` no harness; migration **aditiva, sem ledger e sem
> backfill** (acervo `COUNT = 0`) — `ea0aa4f`. (2) **A régua do saldo no `POST`** (`57ace18`): o
> `pedido_item_id` é resolvido **pelo servidor**, a esperada nasce do **saldo da linha**, e a
> comparação é **por MATERIAL** (linhas do mesmo material somadas). 400 *"Quantidade recebida (N)
> maior que o saldo do pedido (S) para o material COD — …"* com o **mesmo sufixo** do 400 da 36
> (constante compartilhada, para as duas portas não instruírem o operador de formas diferentes), e
> o **mesmo 403**, agora emitido de um lugar só. (3) **Só o que ENTROU no estoque consome saldo**
> (`d062889`): o acumulador roda dentro do claim de `darEntradaEstoque`, nos **dois** caminhos, é
> idempotente e **não-fatal** de propósito — perder a contagem é reversível por SQL, travar a nota
> com o estoque já creditado não é. (4) **A situação do pedido é DERIVADA na leitura**
> (`402070c` + `eb10d9c`): `ABERTO`/`PARCIAL`/`RECEBIDO`, `saldo_pendente` clampado e
> `saldo_pendente_material` — **o mesmo agregado que a barreira usa**, porque sem ele a leitura
> prometia 10 e a porta recusava com 5 —, com `?pendentes=1` filtrando **no `WHERE`, antes do
> `LIMIT`**, e o `status` do core **intocado**. (5) **A tela parou de apagar os itens do pedido**
> (`a9acb4c` + `838f971`): linhas editáveis com o saldo, aviso *"Acima do saldo: …"* que soma o
> digitado **por material** (o fix 1 — medir a linha avisava falso e deixava passar o que a porta
> recusava) e a caixa de autorização só para quem tem a ação, com o backend decidindo. (6) **Os 21
> `ALTER TABLE` mortos apagados** (`4955805`, galho de fundação). (7) **A integração que cruza os
> galhos** (`97726c3`): 10 → parcial de 6 pela forma da tela → `PARCIAL`/saldo 4 → recusa →
> autorização → `RECEBIDO`/saldo 0, com `/conferir` e `/fiscal` ecoando **200 sem trilha nova** (o
> CRÍTICO da 36 não renasceu).
> **A REVISÃO ADVERSARIAL FINAL (2 lentes independentes, 0 ruído) achou 4 Important, e os quatro
> vivem na COMPOSIÇÃO — nenhum dentro de uma task.** **F1** (`4007344`): a tela colapsava *"pedido
> sem itens lançados no Compras"* e *"pedido quitado"* no mesmo `[]` e chamava o pedido **vazio** de
> quitado — a mentira que o design tirou do servidor, renascida no client. **F2** (`8ead95e`): o
> `POST` **sem `itens`** montava o payload com o saldo **por linha** e a barreira agrega **por
> material** — o servidor recusava o **próprio** payload. **F3** (`c40fca8`): o `pedido_item_id`
> era validado contra o pedido mas **não contra o material**, então receber X com o id da linha de
> Y creditava X no estoque e dava baixa em **Y**. **U1** (`dd11972`): o caminho por pedido gravava
> os itens a **R$ 0,00** e **deixou de alimentar o custo médio** — regressão silenciosa da Etapa 8c,
> introduzida por uma task desta etapa e invisível para toda a suíte. Mais **6 Minor** (`e253ad2`,
> `93cce5e`, `e0f8b18`, `13ad237` — este último: `var(--gmp-danger)` **nunca existiu**, só
> `--gmp-error`, e por isso os avisos saíam na cor herdada).
> **⚠️ A DESCOBERTA QUE MAIS IMPORTA NÃO É DEFEITO DESTA ETAPA — é o que ela NÃO cobre:
> NENHUM código da aplicação cria pedido de compra.** Não existe `POST` de pedido (a rota core é
> **GET-only**), "Novo Pedido" cai no `path="*"` e volta para a lista, e o acervo é 0/0. Em
> produção, portanto, a forma "Pedido de compra" abre um `<select>` **vazio e obrigatório**: tudo o
> que esta etapa entregou fica **inerte** até o Compras ganhar a criação. Até lá, **o teste manual
> exige um pedido inserido por SQL** (o `INSERT` pronto está na letra **F**).
> **O que é do usuário:** as consultas **A12** (reconciliar `quantidade_recebida` a partir dos
> recebimentos já processados — só faz sentido depois do deploy, e é a mesma consulta que fecha a
> janela de crash do acumulador) e **A13** (`PRAGMA table_info` das tabelas dos 21 `ALTER` contra o
> schema — deve dar 0 colunas ausentes, e deu); as decisões desta etapa registradas a partir de
> **B89**; e a pergunta da letra **B** sobre o **F13**: o `pedido_item_id` de linha de **outro
> pedido** deve ser recusado? Hoje não é — o F3 recusa só material divergente dentro do pedido, e
> mudar isso é contrato novo.
> **A próxima etapa já está decidida e desenhada:** **Etapa 38 — o pedido de compra ganha criação**
> (módulo core Compras), com importação de planilha como carga inicial e "Gerar pedido" na
> Reposição fechando o elo com `vincularPedidoCompra`. Medição em
> `.superpowers/sdd/etapa38-fase0-pedido-de-compra.md`.
> Antes: 2026-09-16 (**Etapa 36 fechada — as DUAS portas de escrita do
> recebimento passaram a recusar o que sempre aceitaram, `d02b9f4..e287a06`. É FEATURE da 08**
> (servidor + client), e é a primeira etapa de feature depois de três de higiene de tela.
> **Entregue:** (1) **enum** de `tipo_recebimento` validado nas duas portas —
> `POST /almoxarifado/recebimentos` e `PUT /almoxarifado/recebimentos/:id/fiscal` — com fonte única
> em `schema.js` e Zod `z.looseObject` (`z.object` fazia *strip* de `nota_fiscal`/`itens` e viraria
> **todo** POST válido em 400; medido por sonda **antes** de escrever); sonda que gravava
> `'BANANA<script>'` com 201 passou a 400 (`d02b9f4`). (2) **A mesma NF do mesmo fornecedor não
> entra duas vezes** — guarda no **serviço**, antes do `INSERT` e antes do `UPDATE` fiscal, 409
> nomeando **em qual recebimento** a nota já está; dano medido antes do conserto: **20** em vez de
> 10 no estoque e **2** contas a pagar (`ffc5f47`, chave corrigida em `c5f14c8`). (3) **Receber mais
> que o esperado exige autorização** — ação nova `autorizar_excedente`
> **`[ADMINISTRADOR, COMPRAS]`**, checada no serviço nas **três** portas que escrevem quantidade
> recebida, com trilha `EXCEDENTE_AUTORIZADO` por item (`f747df4`, `3e36af4`, `230baf6`, `2d7787d`
> e o F5 da onda, `17c4130`). (4) **A régua do
> workflow** que a spec 08 exigia desde 2026-08-11, zero linhas de produção (`9d19e7d`). (5) **O
> campo "Qtd. conferida" por item no painel**, com o aviso de divergência e o botão "Salvar
> Conferência" — a rota `PUT /:id/conferir` **nunca tinha tido chamador** (`e2a23a9`, `d02744a`,
> `e287a06`). (6) **Itens do POST validados** (`d90853d`) e (7) o resíduo do `loadingDetalhe`
> fechado nas duas telas, **declaradamente sem régua** (`5ce3fdf`). (8) Integração das três guardas
> em cinco pontos de chamada, pela rota **e** pelo serviço (`aa39155`).
> **A REVISÃO ADVERSARIAL FINAL (2 lentes independentes) CONVERGIU NUM CRÍTICO** que a suíte verde
> não via: o documento cujo excedente havia sido **autorizado** na conferência **nunca mais salvava
> dados fiscais** — o modal de NF reenvia a quantidade de todos os itens e não tem caixa nenhuma, a
> barreira tratava o **eco** como ato novo, e o `processar` seguinte morria: **documento preso**. E
> toda linha de acervo com `recebida > esperada` travaria no primeiro deploy. Regra unificada nas
> duas portas: barra só quando `recebida > esperada` **E** `recebida > quantidade já gravada`, e a
> trilha só é escrita quando a barreira disparou e foi autorizada (`230baf6` + `2d7787d`). **E a
> re-revisão da onda achou o efeito colateral do próprio conserto (F5, `17c4130`):** a regra
> certa removeu o bloqueio **acidental** que o `/fiscal` dava a um recebimento criado por API já
> com quantidade acima da esperada — 201 → 200 → 200 → **processável, sem flag, sem permissão e sem
> trilha** —, e a barreira passou a valer também em `criarRecebimento`. Esse defeito **não existia**
> quando as duas lentes rodaram: é o argumento medido a favor de re-revisar conserto de crítico.
> Mais 4 Important e 4 Minor, **0 ruído**: a chave de fornecedor era contornável de **três** jeitos
> (`UPPER` do SQLite é ASCII-only; a comparação de **uma perna só** deixava identificação mista
> passar; CNPJ sem trim no parâmetro — `c5f14c8`); mandar `quantidade_esperada: 'abc'` era a forma
> de **desligar** a barreira em silêncio (`d90853d`); e a tela dizia
> *"Divergência: 0 a mais"* enquanto o servidor barrava o save (`e287a06`).
> **O que é do usuário:** as consultas **A9** (duplicatas de NF no acervo — e ela **SUB-REPORTA**
> nome acentuado, porque o `UPPER` do SQLite não remove diacrítico: o número é **piso**), **A10**
> (`tipo_recebimento` fora do enum) e **A11** (recebimentos abertos com `recebida > esperada`), e a
> decisão **B82**: *o GESTOR deve autorizar excedente?* Se sim, precisa de **rota própria** —
> `receber_material` o exclui, e alargá-la entregaria o fluxo inteiro de recebimento a ele. As
> decisões desta etapa estão registradas a partir de **B80**.
> **A próxima etapa já está decidida, desenhada e commitada:** **Etapa 37 — recebimento parcial e
> excedente CONTRA O PEDIDO DE COMPRA** (a terceira porta: o `POST` ainda aceita 999 de 10, porque
> a barreira compara com a linha já gravada), com a limpeza dos 21 `ALTER TABLE` mortos como galho
> final — design `5f03afc`, plano `9790b07`.
> Antes: 2026-09-16 (**Etapa 35 fechada — os três defeitos que a revisão da
> Etapa 34 achou nas telas VIZINHAS, `6f6a8b0..2d5cd35`. NÃO é feature: nenhuma feature muda de
> cor, e `server/` não mudou uma linha** — medido: `git diff --stat 1cbcf2f..2d5cd35 -- server/`
> volta vazio.
> **Entregue:** (a) **Requisições** — o clique na linha carregava o detalhe **2x** (latência
> dobrada e um piscar de "Carregando" por abertura); fechado com uma **flag de navegação interna
> por identidade de query** (`6f6a8b0`), mais o cenário de integração de quatro gestos que prova
> que a flag **desarma** (`d484458`). (b) **Recebimentos** — os três `catch` de carga engoliam o
> erro e falha de rede aparecia como *"Nenhum recebimento registrado"*; agora a falha vai para o
> **DOM** com *"Tentar de novo"*, a lista é zerada e o erro de materiais aparece dentro do modal
> (`22e1d9b`); e o painel parou de mostrar o registro **anterior** sob o id novo, com a resposta
> fora de ordem descartada (`4681111`) — dois itens que **nenhuma spec tinha** (C46/C47). (c)
> **Transversal às 11 telas do módulo que têm coluna de ações:** `.almox-actions` ganhou
> `flex-wrap: wrap` (`3b23f04` + `8a84ad3`), porque sem ele a célula estourava a largura e o
> `overflow: hidden` de `.almox-table-container` **cortava** os botões sem barra de rolagem e sem
> sintoma. Pior caso medido por aritmética, não por navegador: **Ferramentas** (5 botões de texto
> com `white-space: nowrap`: ~683,5px de célula contra 721px úteis em 769px, com seis colunas ainda
> por caber); Materiais tem 10 ícones (374px). Veio com **teste de drift** (`almoxActionsCss.test.js`,
> lê o CSS como texto, 6 asserções + guarda-da-guarda), porque jsdom não faz layout e nenhum teste
> desta base prova pixel. **Este item (c) não pertence a feature nenhuma** — é CSS compartilhado —,
> e até esta etapa **não estava em lugar nenhum das specs**: só no plano da Etapa 34, como letra F.
> Fica aqui.
> **A REVISÃO ADVERSARIAL desta etapa** (6 gates de task + 2 lentes finais em paralelo, veredito
> Clean nas duas, **0 ruído**) pegou dois achados **Low** que nenhum teste via: (1) `fecharDetalhe`
> de `RequisicoesList.js` não bumpava a sequência do fetch, então o ✕ com o `GET` em voo deixava a
> resposta morta reescrever `?id=` na URL com o painel fechado — e o **F5 do usuário reabria a
> requisição que ele fechou** (`2817054`); (2) `currentStep` de `RecebimentosAlmoxarifado.js` era o
> **oitavo** consumidor de `detalhe` (a tabela de sete do design **estava incompleta**), e a barra
> de etapas piscava para o passo 1 durante a carga — agora fica neutra (`29cbdfa`). Três edições de
> comentário/organização fecharam a onda (`2d5cd35`); quatro achados ficaram **parked** na letra G.
> **O que é do usuário:** a verificação manual **F12**, cuja redação anterior **estava errada na
> tela** (dizia "conferir se o clipe de **Materiais** fica cortado" — o pior caso é **Ferramentas**)
> e está **vencida** (o CSS já mudou): o que resta medir no navegador é o **teto da faixa**
> (~1100–1355px) e as linhas que passaram a quebrar em **duas fileiras**, inclusive abaixo de 768px.
> E as sete decisões desta etapa, registradas a partir de **B72**.
> Antes: 2026-09-16 (**Etapa 34 fechada — os anexos saíram da inspeção e foram
> para as outras CINCO telas que os esperavam, `746a106..054f727`. Nenhuma feature muda de cor, e
> `server/` não mudou uma linha** (medido: `git diff --stat` do range restrito a `server/` vem
> vazio). O mecanismo de anexos nasceu completo na Etapa 32 e ficou com **um** consumidor; esta
> etapa plugou os cinco que faltavam, fechando as seis chaves do mapa `ENTIDADES_ANEXO`.
> **Entregue:** `AnexosModal` (casca de modal, `746a106`) + clipe por linha em **Materiais**
> (`eb89b5e`), **Devoluções** (`67f2389`, que ganhou a primeira coluna de ações que já teve) e no
> **ITEM** da remessa a terceiros (`dd6e4c5`, id do item e não da remessa); bloco inline no painel
> de **Requisições** (`d5578e6`) e de **Recebimentos** (`01dd3ce`, que ganhou a **primeira suíte de
> teste** dessa tela, hoje com 7 cenários); e um teste de integração cruzando as seis chaves do
> client com o mapa do servidor (`656467c`).
> **A REVISÃO ADVERSARIAL (3 revisores em paralelo, 5 achados, 0 ruído) pegou dois Important, e
> nenhum teste os via porque nenhum teste exercitava aqueles caminhos.** (1) O bloco da requisição
> nascia **sem gate de `warehouseMode`** — a mesma `RequisicoesList.js` roda em seis módulos sem
> permissão do almoxarifado, e ali o painel pedia `GET /api/almoxarifado/anexos` → **403 "Acesso
> negado ao módulo"** em vermelho, formulário de upload morto (o hook de permissões falha **aberto**
> de propósito) e **uma linha de auditoria de acesso negado por painel aberto**; `a88d715`.
> (2) O corpo dos dois painéis (`RequisicoesList.js:876`, `RecebimentosAlmoxarifado.js:496`) vivia
> dentro do ternário de `loadingDetalhe` e **desmontava a cada refetch** — inclusive no foco da
> janela, que é exatamente o que acontece ao FECHAR o diálogo de escolher arquivo: o arquivo
> recém-escolhido sumia; `c5d9e99`, com cenário de **identidade do nó DOM** e contagem `=== 1`
> depois do refetch. Um Minor virou comentário no teste de integração dizendo o que ele **não**
> prova (`054f727`); dois Minors ficaram **parked**.
> **Decisão minha na letra B: B71** — o bloco de anexos da requisição aparece **só no modo
> almoxarifado**. Escolhido o gate por `warehouseMode` (reversível, uma linha); **descartado**
> expor anexos nas telas cross-módulo, que exigiria rota nova fora da permissão do módulo (mudança
> de contrato, etapa própria). **Custo se estiver errado:** quem requisita fora do almoxarifado não
> vê nem anexa documento na própria requisição até essa etapa existir.
> **O que é do usuário:** responder a B71 (o solicitante do comercial/engenharia **deve** poder
> anexar desenho à própria requisição?) e a verificação manual da **letra F** — em Materiais a
> coluna de ações não tem `flex-wrap` e o contêiner da tabela é `overflow:hidden`, então o 10º
> ícone **pode** ficar clipado numa faixa de largura de desktop; o breakpoint **não foi medido**,
> porque não há navegador aqui.
> Antes: 2026-09-03 (**Etapa 33 fechada — os uploads legados param de ser
...
| 04 | [Requisições](04-requisicoes/README.md) | 🟢 | 🟢 | 🟢 | 🟢 Etapa 3 entregue (2026-08-05): ciclo ponta a ponta rascunho→envio→aprovação→separação→retirada→entrega→confirmação→encerramento; entrega/estorno via motor de estoque; máquina de estados explícita; falta lote/série na entrega, ~~anexos~~ (**pagos na Etapa 34**) e importação de BOM/OP. Correção 2026-08-11 (`92fe236`): a tela não conhecia os status de reserva da Etapa 4 — requisição aprovada com saldo ficava com badge cru, sem "Iniciar Separação" e sem "Cancelar"; corrigido com teste (`RequisicoesList.test.js`). **Etapa 34 (2026-09-16, `746a106..054f727`):** anexos na tela — bloco **Anexos** no fim do painel de detalhe, entidade `requisicao` com o id do detalhe carregado (`d5578e6`), **só no modo almoxarifado** (`a88d715`, B71) e fora do ternário de loading para não remontar a cada refetch (`c5d9e99`). Furo pré-existente descoberto aqui: abrir pelo clique carrega o detalhe **2x** (`syncSearchParams` reacende o deep-link) — item (a) da Etapa 35. **Etapa 35 (2026-09-16, `6f6a8b0..2d5cd35`):** aquele furo **fechado** (C45) — uma carga por gesto, por flag de navegação interna por identidade de query (`6f6a8b0`), travada pelo `expect(cargasDoDetalhe()).toHaveLength(1)` do cenário do clique e pelo cenário de integração de quatro passos que prova o desarme (`d484458`); e o ✕ passou a descartar a resposta em voo, que antes reescrevia `?id=` na URL com o painel fechado e fazia o F5 reabrir a requisição (`2817054`). Continua 🟢; falta o mesmo de antes (lote/série na entrega, BOM/OP, gestor responsável) e back/forward do navegador segue **sem régua** |
...
| 18 | [Reposição e estoque mínimo](18-reposicao-estoque-minimo/README.md) | 🟢 | 🟢 | ✅ | 🟢 **Etapa 11 entregue (2026-08-24, `54e1278..1ea6ab2`)** — motor de sugestão no `purchaseService` (consumo médio por `TIPOS_SAIDA` em janela configurável; ponto efetivo com **a mínima como chão** de todas as réguas; posição = `disponivelSql` + solicitações abertas dentro do **horizonte** configurável, com `a_caminho_vencido` exposto; alvo `max(máxima, ponto)` com lote econômico como piso), `GET /reposicao/sugestoes` consolidado por fornecedor e valorado, `POST /gerar-solicitacoes` (quantidades do servidor, sem dedupe — a posição É o dedupe, complemento em pendência insuficiente, auditado), `GET /estoque-parado` (excesso/sem consumo/obsoleto com valor parado), ação nova `gerenciar_reposicao` [ADMIN, GESTOR, COMPRAS — ALMOXARIFE fora de propósito], relatório de solicitações com VINCULADO e gateado, horizonte compartilhado com a máquina de estados de requisição, 3 configs semeadas+editáveis com validação nos dois lados, índice novo no livro, tela `/almoxarifado/reposicao` (3 abas, painel de erro/permissão por aba). **Fica de fora, declarado:** ~~fechar/cancelar solicitação no recebimento~~ (**entregue na Etapa 14** — RECEBIDA automática + CANCELADA com justificativa, ver feature 22), ~~criar pedido de compra real~~ (**entregue na Etapa 38**), itens por material (feature 24), alerta ativo de máximo, e-mail (19). **Etapa 38 (2026-09-16, `727ee29` + `ca7956c`) — o ciclo da reposição finalmente TERMINA em pedido.** A aba Solicitações ganhou **"Gerar pedido"** em cada linha `PENDENTE`: é o **primeiro consumidor que `purchaseService.vincularPedidoCompra` já teve** desde a Etapa 11 (a função exigia um pedido que nenhum código criava). O botão só aparece com `gerenciar_reposicao` **e** com o módulo Compras (cache do menu, TTL 5 min, **falha aberto** — quem decide é o servidor), e o clique **navega, não posta** (`/compras/pedidos/novo?solicitacao=…&material=…&quantidade=…`; descartado criar um `GET /solicitacoes/:id` só para pré-preencher — B107). O `POST` do Compras exige `gerenciar_reposicao` **antes de escrever** quando vem `solicitacao_id` (`3e43069`), e a linha só vira `VINCULADO` ao **reabrir** a tela. E **apagar o pedido devolve a solicitação para a fila** (`ca7956c`, achado I1 da revisão final): `PENDENTE` com `pedido_compra_id NULL`, exceto `RECEBIDA`/`CANCELADA` — antes ela ficava `VINCULADO` apontando para um id apagado e **nunca mais virava pedido**. **Segue 🟢** no que é do almoxarifado; o único aberto continua sendo **itens por material** no core (feature 24) |
...
| 20 | [Alertas operacionais](20-alertas/README.md) | 🟢 | 🟢 | ✅ | 🟢 **17 de 20 (Etapa 16 somou 7 por varredura, `6bed5e2..ed5f032`; Etapa 17 somou 4 no ato/vigília, `d65d81b..e51ca79`)** — registro único `alertRegistry` com DOIS modos: varredura diária pela fila da 19 e **disparo no ato** (`dispararAlertaRegistrado`) para reprovação de material, divergência de recebimento (nos dois escritores reais da quantidade) e divergência de inventário (agregada por conferência, sem valor em reais — B30); `listar` dual-mode dá a régua única para gancho e central. Central `/almoxarifado/alertas` ao vivo gateada por `ver_alertas` (B28); 4 janelas configuráveis nos dois lados. Dois itens SAÍRAM do checklist dizendo por quê (transferência: CORTADA pelo cliente; negativo: regra do motor). **Etapa 39 (2026-09-17, `39ea9d2..19ebf7d`) somou o 18º: `PEDIDO_COMPRA_ATRASADO`** (`ddbc18f`, com `33031ac` e `8f3db94` da onda) — **a primeira entrada do registro que lê tabelas CORE** (`pedidos_compra`/`fornecedores`), importando a régua de atraso do módulo Compras por require **lazy** (o ciclo de módulos está a **um** require de fechar, e quando fechar um lado captura `{}` e o cartão vira `erro: true` em silêncio). Colunas **projetadas**, nunca `SELECT p.*` — com `p.*`, quem tem `ver_alertas` mas **não** tem o módulo Compras recebia `valor_total` e `observacoes` de pedido CORE na central; dedupe **`pedido-atrasado-<id>-<previsao_entrega>`**, porque só com o id o pedido era calado para sempre e **prazo renegociado e furado de novo nunca gerava segundo aviso**; assunto com prefixo **`[Compras]`**, e não `[Almoxarifado]`, porque a lista de destinatários é compartilhada. **Falta o que não tem dado OU não tem fonte exportada:** separado-aguardando-retirada (falta a data da transição), consumo-acima-do-previsto (projeto sem orçamento) e **pedido-parcial — e aqui a spec desta feature ESTAVA DESATUALIZADA e foi corrigida à vista**: ela dizia "falta noção de saldo do pedido", o que deixou de ser verdade nas Etapas 37/38; o bloqueio real é que a derivação da 37 **não é exportada** e a fonte que é tem `LIMIT 50` e não devolve `previsao_entrega` — follow-up é fatia da feature 08 |
...
2. ✅ **DDL duplicado** — resolvido. `CREATE TABLE` vive só em `schema.js` desde a Etapa 0, e os
   **21 `ALTER TABLE` com erro engolido** de `routes/almoxarifado.js` (achado 2026-08-11) saíram na
   **Etapa 37 Task 6**: eram **todos os 21** código morto (o `schema.js` cria as 21 colunas via
   `safeAlter`) e falhavam em todo boot porque o `initSchema` não é awaited. O guardião
   `schemaUnico.api.test.js` agora varre `ALTER TABLE` fora de `safeAlter(`, não só `CREATE TABLE`.
   Sobra aberto, e é outro assunto: `initSchema` **não-awaited** (`routes/almoxarifado.js:238`).
