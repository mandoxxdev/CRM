...
- [x] Solicitar análise da Engenharia / devolução ao fornecedor / substituição (encaminhamentos com status) — **Etapa 5, parcial** (`dc841f2`): o campo `encaminhamento` (`DEVOLVER` | `ANALISE_ENGENHARIA` | `SUBSTITUICAO`) é validado e registrado na reprovação. O que fica de fora — **e é a pendência que esta etapa cria**, ver seção própria abaixo — é o "status" do encaminhamento e a execução da saída em si (feature 12, Devoluções). Hoje é só o registro da intenção.
