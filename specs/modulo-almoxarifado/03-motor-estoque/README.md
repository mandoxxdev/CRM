...
> **Última atualização:** 2026-08-09 (Etapa 6, Task 3, round 4 do review — estorno não cria linha de saldo; mapa volta a mostrar material sem endereço; correção de uma afirmação errada desta spec sobre a invariante da linha de saldo)
...
- **Pendência nomeada — a conclusão de inventário escreve `quantidade_atual` por fora do motor.**
  ...
  `UPDATE materiais_almoxarifado SET quantidade_atual = ?` direto (`routes/almoxarifado.js`, ~linha
  868, com um comentário no próprio arquivo dizendo que é "o caminho mais destrutivo do arquivo") e
  insere a linha `AJUSTE` no livro à mão. **Nunca toca em `estoque_saldo_almoxarifado`.** Com a
