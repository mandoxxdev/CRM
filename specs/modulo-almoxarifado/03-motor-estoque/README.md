...
- **Saldos:** `estoque_saldo_almoxarifado` por material+localização+lote; espelho agregado do FÍSICO em `materiais_almoxarifado.quantidade_atual` (`syncMaterialTotals`, chamado pelo AJUSTE com localização e pelo estorno dele). A tabela tem colunas `quantidade_reservada/bloqueada/em_inspecao` no `CREATE TABLE`, mas **nada as escreve** — retenção é por material, não por localização. `syncMaterialTotals` recalcula **só** `quantidade_atual` por isso: recalcular retenção a partir dessa soma sempre-zero zerava a quarentena e o hold das reservas do material a cada AJUSTE por localização (achado do review final da Etapa 5).
